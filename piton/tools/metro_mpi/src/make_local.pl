...
$n=0;
process_flist($flist_in,$dirname);

sub process_flist {
    my ($flist_in,$dirname)=@_;
    print "\n--Processing file list: $flist_in on $dirname\n";
    ...
    foreach my $line (<FLIST_IN>)  {   
        ...
        $line =~ s/\s+\/\/.*//g;
	    $line =~ s/^\s*\/\/.*//g;
	    $line=replace_linux_var($line);
	    #replace ./ with $dirname
	    $line =~ s/^\.\//$dirname\//g;
	    #replace ./ with $dirname
	    $line =~ s/^-F\s+\.\//-F $dirname\//g;
	    $line =~ s/^-f\s+\.\//-f $dirname\//g;
	    $line =~ s/^\+incdir\+\./\+incdir\+$dirname/g;
	    #remove /./ 
	    $line =~ s/\/\.\//\//g;  # Removes /. from file path
        ...
        #next;
            ...
            my ($fdir) = $fname =~ m{^(.*)/[^/]+\.[^.]+$};  # Removes the file name to leave just the path    
            
            process_flist($fname,"$fdir/");
...
sub replace_linux_var{
    my $line=shift; 
    my @captured_strings = $line =~ /\$(\w+)|\$\{(\w+)\}/g;        
    foreach my $var (@captured_strings) {
        next if (!defined $var);
        my $replace=$ENV{$var};
        die "Error: undefined $var linux variable\n" if(!defined $replace);
        $line =~ s/\$$var|\$\{$var\}/$replace/g;
    }
    return $line;
}
